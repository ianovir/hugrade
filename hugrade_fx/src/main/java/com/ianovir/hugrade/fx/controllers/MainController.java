...
import com.ianovir.hugrade.core.converters.Graph2GMLConverter;
...
import com.ianovir.hugrade.fx.views.*;
...
public class MainController implements GraphView.SelectionObserver, GraphChangeObserver {
    ...
    private void actionFileSaveAs() {
        ...
        if (file != null) {
            graphFileWriter.writeGraph(graphView.getGraph(), file);
            currentFile = file;
        }
    }
    ...
    private void actionFileSave(javafx.event.ActionEvent e) {
        if(graphView==null) return;
        if(currentFile!=null)
            graphFileWriter.writeGraph(graphView.getGraph(), currentFile);
        else
    ...
    private void actionOpenFile() {
        ...
        if (file != null) {
            graphView = new GraphView(graphFileReader.readGraph(file));
            graphView.addSelectionObserver(this);
            currentFile = file;
            onGraphChanged(graphView);
        }
